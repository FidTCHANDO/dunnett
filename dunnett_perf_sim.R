if (!require(multcomp)) install.packages('multcomp',dependencies = T)
...
post.hoc.perf <- function(ng = c(3:5),                 # number of groups
                          ...
                          dist = c("N"), # distribution
                          level = 0.05
){
  totsim <- length(ng)*((length(gs)+1)*(4))*sim
  
  postname <- c("GN","Bal","GS","DT","Mean","Variance","Sim","nb","dunnet","xdunnet")
  ...
  for (i in ng) { # groups number
    
    for (j in 1:2) { # unbalanced or balanced group sizes
      ...
      for (k in siz) {
        ...
        for (l in dist) { # dist
          ...
          for (m in 1:2) { # group means
            
            if (m == 1) mg <- rep(2, i) else mg <- 2+1:i*1
            
            for (n in 1:2) { # variances
  ...
  close(pb)
  for (i in names(NormRes)[c(1,5,8:10)]) {
...
options(warn = 0)
# system.time(dset <- post.hoc.perf(ng = sort(c(3, 5),T),
#                                   gs = sort(c(5, 10),T),
#                                   sim = 2))
...
plotting <- function(meas.estim, distri = names(meas.estim), kind = levels(as.factor(meas.estim[[1]][[2]]$kind))){
  ...
  gsize <- sort(unique(gen.gsize(meas.estim$N$data)))[c(7,8,11,9,10,12,13:15,18,16,17,19:21,23,22,24:26,28,27,29:30,1:6)]
